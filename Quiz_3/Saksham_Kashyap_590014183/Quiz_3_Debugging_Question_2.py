...
Solution: the bug is in Line B, current +=1.

2. Explain why Line A or Line B causes the incorrect behavior
Solution: Line A does not Causes any incorrect behavior but Line B does Causes the incorrect behavior, Because after swapping nums[current] with nums[right], the new value at nums[current] is not yet evaluated. 
